...
const contactLinks = [
  { icon: '@', href: 'mailto:alex@example.com', label: 'alex@example.com' },
  { icon: '</>', href: 'https://github.com', label: 'github.com/alexchen' },
  { icon: 'in', href: 'https://linkedin.com', label: 'linkedin.com/in/alexchen' },
]
...
export default function Contact() {
  ...
  return (
    <>
      ...
      <section style={{ padding: '5rem 3rem 7rem', maxWidth: '1100px', margin: '0 auto' }}>
        <p style={{ fontFamily: 'var(--font-mono)', fontSize: '0.72rem', color: 'var(--cyan)', letterSpacing: '0.3em', textTransform: 'uppercase', marginBottom: '0.8rem' }}>04 / Contact</p>
        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(2.5rem,5vw,4rem)', fontWeight: 800, color: '#fff', lineHeight: 1.1, marginBottom: '4rem' }}>
          Let's build something<br />that matters
        </h2>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '5rem', alignItems: 'start' }}>
          <div>
            <p style={{ color: 'var(--text-muted)', fontWeight: 300, fontSize: '1.05rem', marginBottom: '1.3rem' }}>
              Whether you're looking to bring on a backend engineer, explore AI infrastructure challenges, or collaborate on a research project — I'm always open to interesting conversations.
            </p>
            <p style={{ color: 'var(--text-muted)', fontWeight: 300, fontSize: '1.05rem', marginBottom: '2rem' }}>
              Fastest response via email. Connect on GitHub to see what I'm building.
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {contactLinks.map(({ icon, href, label }) => (
                <a key={label} href={href} className="contact-link" target={href.startsWith('http') ? '_blank' : undefined}>
                  <span style={{ color: 'var(--cyan)', width: '20px', textAlign: 'center', fontFamily: 'var(--font-mono)', fontSize: '0.85rem' }}>{icon}</span>
                  {label}
                </a>
              ))}
            </div>
